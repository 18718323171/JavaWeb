function update() { // 用户信息修改按钮事件
	...
	// 联系电话
	updateTxtStyle($("#txtIdCard"));
}
function updateCancel() { // 用户信息取消按钮事件
	var txtName = $("txtName");
	if (txtName.attr("readonly") != "readonly") { // 如果readonly的值不为"readonly"则表示正处于
													// 修改状态
		// 姓名
		...
		// 联系电话
		updateCancelTxtStyle($("#txtIdCard"));
	}
}
...
/*
 * function test_hiddenOp(param1){ //语文、英语、数学题框的显隐操作 //alert(param1); if(param1 ==
 * "Chinese_test"){ $("#"+param1).css("display", "");
 * $("#English_test").css("display","none"); $("#Math_test").css("display",
 * "none");
 * 
 * }else if(param1 == "English_test"){ $("#"+param1).css("display", "");
 * $("#Chinese_test").css("display","none"); $("#Math_test").css("display",
 * "none"); }else if(param1 == "Math_test"){ $("#"+param1).css("display", "");
 * $("#Chinese_test").css("display","none"); $("#English_test").css("display",
 * "none"); } //ulShow(); }
 */
/*
 * $(function(){ $("#Chinesetest_addBtn").click(function(){ $(this).hide();
 * $("#Chinesetest_addPane").show(); });
 * $("#Chinesetest_addCancel").click(function(){
 * $("#Chinesetest_addBtn").show(); $("#Chinesetest_addPane").hide(); });
 * $("#Mathtest_addBtn").click(function(){ $(this).hide();
 * $("#Mathtest_addPane").show(); }); $("#Mathtest_addCancel").click(function(){
 * $("#Mathtest_addBtn").show(); $("#Mathtest_addPane").hide(); });
 * $("#Englishtest_addBtn").click(function(){ $(this).hide();
 * $("#Englishtest_addPane").show(); });
 * $("#Englishtest_addCancel").click(function(){
 * $("#Englishtest_addBtn").show(); $("#Englishtest_addPane").hide(); }); });
 */